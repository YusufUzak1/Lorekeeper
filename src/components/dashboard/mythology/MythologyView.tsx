...
export function MythologyView() {
  const { getMythsForCurrentUniverse, deleteMyth } = useUniverseStore();
  const { showConfirm } = useConfirmStore();
  const myths = getMythsForCurrentUniverse();
  const scrollRef = useRef<HTMLDivElement>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingMyth, setEditingMyth] = useState<MythCard | null>(null);
  ...
  const handleEdit = (myth: MythCard, e: React.MouseEvent) => {
    e.stopPropagation();
    setEditingMyth(myth);
    setIsModalOpen(true);
  };
  ...
  const handleDelete = (id: string, name: string, e: React.MouseEvent) => {
    ...
    showConfirm({
      title: 'Efsaneyi Sil',
      message: `"${name}" efsanesini kalıcı olarak silmek istediğinize emin misiniz?`,
      danger: true,
      confirmText: 'Sil',
      onConfirm: () => deleteMyth(id),
    });
  };

  const handleCloseModal = () => {
    setIsModalOpen(false);
    setTimeout(() => setEditingMyth(null), 300);
  };
                ...
                {categoryMyths.map((myth, idx) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-6 relative z-10">
                      ...
                      {/* Sağ Taraf: Sınıf/Tier ve Butonlar */}
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={(e) => handleEdit(myth, e)} className="p-1.5 bg-[#0a0a0b] hover:bg-white/10 rounded border border-white/10 text-white/40 hover:text-white transition-colors" title="Düzenle">
                            <Pencil size={12} />
                          </button>
                          <button onClick={(e) => handleDelete(myth.id, myth.name, e)} className="p-1.5 bg-[#0a0a0b] hover:bg-red-500/20 rounded border border-white/10 text-white/40 hover:text-red-400 transition-colors" title="Sil">
                            <Trash2 size={12} />
                          </button>
                        </div>
                      </div>
