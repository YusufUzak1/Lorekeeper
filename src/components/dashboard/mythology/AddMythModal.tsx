...
interface AddMythModalProps {
  isOpen: boolean;
  onClose: () => void;
  editItem?: MythCard | null;
}

export function AddMythModal({ isOpen, onClose, editItem }: AddMythModalProps) {
  const { addMyth, updateMyth } = useUniverseStore();
  ...
  useEffect(() => {
    if (editItem && isOpen) {
      ...
      setEpithet(editItem.epithet);
      setTier(editItem.tier || TIER_OPTIONS[0]);
      setColor(editItem.color);
      setGlyph(editItem.glyph);
      setDescription(editItem.description || '');
      setDomainsInput(editItem.domains.join(', '));
    } else if (!isOpen) {
      // Clear handled by onClose
    }
  }, [editItem, isOpen]);
              ...
              <div className="relative flex items-center justify-between px-8 pt-8 pb-4">
                <div>
                  <h2 className="font-serif text-xl tracking-[0.15em] text-[#E8D48B] uppercase">
                    {editItem ? 'Efsaneyi Düzenle' : 'Yeni Efsane Yarat'}
                  </h2>
                  <p className="text-[0.65rem] text-white/30 mt-1 tracking-wider">
                    {editItem ? 'Bu efsanevi varlığın detaylarını güncelleyin.' : 'Panteon\'a yeni bir tanrı, varlık veya efsane ekleyin.'}
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="px-8 pb-8 flex flex-col gap-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <label className="flex flex-col gap-1.5">
                    ...
                    <div className="flex flex-col gap-2">
                      <select
                        value={TIER_OPTIONS.includes(tier) ? tier : 'Özel'}
                        onChange={(e) => {
                          if (e.target.value !== 'Özel') {
                      ...
                      {(!TIER_OPTIONS.includes(tier) || tier === 'Özel' || tier === '') && (
                        <motion.input
                          initial={{ opacity: 0, height: 0 }}
                          animate={{ opacity: 1, height: 'auto' }}
                          exit={{ opacity: 0, height: 0 }}
                          type="text"
                          value={tier === 'Özel' ? '' : tier}
                          onChange={(e) => setTier(e.target.value)}
                          placeholder="Kendi sınıfınızı/kategorinizi girin..."
                          className="w-full rounded-lg bg-white/[0.03] border border-mythos-accent/30 px-4 py-2.5 text-sm text-white/90 outline-none focus:border-mythos-accent/70 focus:ring-1 focus:ring-mythos-accent/50 transition-all placeholder:text-white/15"
                          autoFocus
                        />
                      )}
                    </div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  >
                    ...
                    {editItem ? 'Değişiklikleri Kaydet' : 'Efsaneyi Yarat'}
                  </button>
                </div>
