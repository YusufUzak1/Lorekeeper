...
import { LayoutGrid, List, Search, CloudDownload, CloudUpload, Loader2 } from 'lucide-react';
...
export function Topbar() {
  ...
  const handlePullFromCloud = async () => {
    try {
      ...
      console.error(err);
      alert('Buluttan veri çekilemedi.');
    } finally {
      setIsPulling(false);
    }
  };
  ...
  const handlePushToCloud = async () => {
    try {
      ...
      console.error(err);
      alert('Buluta veri gönderilemedi.');
    } finally {
      setIsPushing(false);
    }
  };
  ...
  return (
    <div className="h-14 flex items-center shrink-0 px-4 border-b border-glass-border bg-[#0a0a0b]/85 gap-3">
      ...
      {/* Cloud Sync Buttons */}
      <div className="flex items-center gap-2 border-r border-glass-border pr-4 mr-1">
        <button
          onClick={handlePushToCloud}
          disabled={isPushing || isPulling}
          title="Yerel Verileri Buluta Yedekle"
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-mythos-accent/20 bg-mythos-accent/5 hover:bg-mythos-accent/10 text-mythos-accent/70 hover:text-mythos-accent transition-all cursor-pointer font-sans text-[0.6rem] uppercase tracking-wider disabled:opacity-50"
        >
          {isPushing ? <Loader2 size={13} className="animate-spin" /> : <CloudUpload size={13} />}
          <span>Buluta Gönder</span>
        </button>
        <button
          onClick={handlePullFromCloud}
          disabled={isPushing || isPulling}
          title="Yapay Zeka Değişikliklerini Buluttan Çek"
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-blue-500/20 bg-blue-500/5 hover:bg-blue-500/10 text-blue-400/70 hover:text-blue-400 transition-all cursor-pointer font-sans text-[0.6rem] uppercase tracking-wider disabled:opacity-50"
        >
