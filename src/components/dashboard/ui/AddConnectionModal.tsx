...
import { X, Link as LinkIcon, Users, Swords, CircleDot, MapPin, Zap, HelpCircle } from 'lucide-react';
import { useUniverseStore } from '@/store/useUniverseStore';
import type { RelationType } from '@/types';

const RELATION_OPTIONS: { value: RelationType; label: string; icon: any; color: string }[] = [
  { value: 'friend', label: 'Dostluk', icon: <Users size={14} />, color: 'text-emerald-400' },
  { value: 'enemy', label: 'Düşmanlık', icon: <Swords size={14} />, color: 'text-red-400' },
  { value: 'neutral', label: 'Nötr', icon: <CircleDot size={14} />, color: 'text-gray-400' },
  { value: 'located_in', label: 'Konum', icon: <MapPin size={14} />, color: 'text-amber-400' },
  { value: 'involved_in', label: 'Katılım', icon: <Zap size={14} />, color: 'text-purple-400' },
  { value: 'other', label: 'Diğer', icon: <HelpCircle size={14} />, color: 'text-gray-400' },
];
