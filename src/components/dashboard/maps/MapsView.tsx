...
import { Compass, ZoomIn, ZoomOut, Maximize, Plus, Map as MapIcon, Pencil, Trash2 } from 'lucide-react';
import type { MapRegion } from '@/types';
import { AddMapRegionModal } from './AddMapRegionModal';

export function MapsView() {
  const { regions, currentUniverseId, deleteRegion } = useUniverseStore();
  ...
  const handleEdit = (region: MapRegion, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleDelete = (id: string, name: string, e: React.MouseEvent) => {
    ...
    showConfirm({
      title: 'Bölgeyi Sil',
      message: `"${name}" bölgesini haritadan kalıcı olarak silmek istediğinize emin misiniz?`,
      danger: true,
      confirmText: 'Sil',
  ...
  const handleCloseModal = () => {
    ...
    setTimeout(() => setEditingRegion(null), 300);
  };
  ...
  return (
    <div className="h-full w-full bg-[#0a0a0b] flex flex-col md:flex-row relative overflow-hidden" onMouseLeave={onMouseLeave}>
      ...
      <div className="w-full md:w-72 h-48 md:h-full bg-black/40 border-b md:border-b-0 md:border-r border-white/10 z-10 flex flex-col backdrop-blur-md">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col gap-2" data-lenis-prevent="true">
          {currentRegions.length === 0 ? (
            ...
            currentRegions.map(region => (
              <div 
                ...
                className={`group p-3 rounded border transition-all cursor-pointer flex justify-between items-center ${
              ...
              >
                ...
                <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-auto pl-2">
                   <button onClick={(e) => handleEdit(region, e)} className="p-1 hover:bg-white/10 rounded text-white/40 hover:text-white transition-colors" title="Düzenle">
                     <Pencil size={10} />
                   </button>
                   <button onClick={(e) => handleDelete(region.id, region.name, e)} className="p-1 hover:bg-red-500/20 rounded text-white/40 hover:text-red-400 transition-colors" title="Sil">
                     <Trash2 size={10} />
                   </button>
                </div>
              </div>
