...
export function CosmosNode({ entity, position, isActive, isFaded, onClick, onHover }: CosmosNodeProps) {
  ...
  const baseColor = COLOR_MAP[entity.type as keyof typeof COLOR_MAP] || '#ffffff';
  const radius = RADIUS_MAP[entity.type as keyof typeof RADIUS_MAP] || 1;
        ...
        >
          <div className="pointer-events-none select-none transition-all duration-300"
               style={{
                 ...
                 color: highlight ? accentColor : 'rgba(200,200,220,0.5)',
                 textShadow: highlight ? `0 0 15px ${accentColor}` : '0 0 10px rgba(0,0,0,0.8)',
                 whiteSpace: 'nowrap',
               }}>
            {entity.name}
          </div>
