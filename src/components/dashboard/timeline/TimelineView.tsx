...
export function TimelineView() {
  const { timeline, currentUniverseId, deleteTimelineEvent } = useUniverseStore();
  ...
  const handleEdit = (event: TimelineEvent, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleDelete = (id: string, name: string, e: React.MouseEvent) => {
    ...
    showConfirm({
      title: 'Olayı Sil',
      message: `"${name}" olayını zaman çizelgesinden kalıcı olarak silmek istediğinize emin misiniz?`,
      danger: true,
      confirmText: 'Sil',
      onConfirm: () => deleteTimelineEvent(id),
    });
  };
        ...
        >
          ...
          {eventsInActiveEra.map((ev, index) => (
             ...
             >
               ...
               {/* Kart */}
               <div 
                 className="bg-[#0D0D0E]/90 border border-white/[0.05] p-3 max-w-[155px] min-w-[125px] transition-all duration-300 group-hover:border-mythos-accent/30 group-hover:shadow-[0_0_22px_rgba(0,0,0,0.8),0_0_14px_rgba(212,175,55,0.07)] relative"
                 style={ev.side === 'above' ? { marginBottom: '4px' } : { marginTop: '4px' }}
               >
                 {/* Aksiyon Butonları (Edit / Delete) - Hoverda görünür */}
                 <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-30">
                    <button onClick={(e) => handleEdit(ev, e)} className="p-1.5 bg-[#0a0a0b] hover:bg-white/10 rounded-full border border-white/10 text-white/40 hover:text-white transition-colors" title="Düzenle">
                      <Pencil size={10} />
                    </button>
                    <button onClick={(e) => handleDelete(ev.id, ev.name, e)} className="p-1.5 bg-[#0a0a0b] hover:bg-red-500/20 rounded-full border border-white/10 text-white/40 hover:text-red-400 transition-colors" title="Sil">
                      <Trash2 size={10} />
                    </button>
                 </div>
