...
export function AddTimelineEventModal({ isOpen, onClose, defaultEra, editItem }: AddTimelineEventModalProps) {
  const { addTimelineEvent, updateTimelineEvent } = useUniverseStore();
  ...
  const handleSubmit = () => {
    ...
    if (editItem) {
      updateTimelineEvent(editItem.id, {
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="font-serif text-lg tracking-[0.1em] text-[#E8D48B] uppercase flex items-center gap-2">
                    <Clock size={18} /> {editItem ? 'Zaman Çizelgesi: Olay Düzenle' : 'Zaman Çizelgesi: Olay Ekle'}
                  </h2>
                  <p className="text-[0.65rem] text-white/40 tracking-wider mt-1">{editItem ? 'Tarihin akışındaki bu dönüm noktasını güncelleyin.' : 'Tarihin akışına yeni bir dönüm noktası kazıyın.'}</p>
                </div>
                <button
                  type="button"
                  onClick={handleClose}
              ...
              <div className="flex flex-col gap-6">
                ...
                >
                  <Plus size={14} />
                  {editItem ? 'Değişiklikleri Kaydet' : 'Olayı Çizelgeye Ekle'}
                </button>
