...
function App() {
  ...
  useEffect(() => {
    const handleMessage = (e: MessageEvent) => {
      if (e.data?.type === 'theme-color-change') {
        setAccentColor(e.data.color);
      }
    };
    window.addEventListener('message', handleMessage);
    
    // Initial load from store is already handled in store definition
    const savedColor = useThemeStore.getState().accentColor;
    if (savedColor) {
      document.documentElement.style.setProperty('--color-mythos-accent', savedColor);
    }
