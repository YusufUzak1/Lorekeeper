...
export const fetchStateFromCloud = async () => {
  ...
  if (!response.ok) {
    throw new Error('Bulut verisi çekilirken hata oluştu.');
  }

  return response.json();
};

export const pushStateToCloud = async (stateData: any) => {
  const session = await fetchAuthSession();
  const token = session.tokens?.idToken?.toString();
  ...
  if (!response.ok) {
    throw new Error('Veri buluta gönderilirken hata oluştu.');
  }
